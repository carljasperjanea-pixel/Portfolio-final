...
import { Menu, X, Github, Linkedin, Mail, FileText, ExternalLink } from 'lucide-react';
          ...
          <div className="hidden md:flex items-center space-x-8">
            {navItems.map((item) => (
              <a
                key={item.name}
                href={item.href}
                className="text-gray-600 hover:text-brand-dark transition-colors font-medium text-sm uppercase tracking-wider"
              >
                {item.name}
              </a>
            ))}
            <a
              ...
              className="bg-brand-dark text-white px-4 py-2 rounded-full text-sm font-medium hover:bg-brand-medium transition-colors"
            >
              Let's Talk
            </a>
          </div>
